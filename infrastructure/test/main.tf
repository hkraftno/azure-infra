...
// Applications Subnet & Proxy Server
...
resource "azurerm_virtual_machine" "proxyvm" {
  name                          = "${var.env}ProxyVM"
  location                      = "${azurerm_resource_group.resourcegroup.location}"
  resource_group_name           = "${azurerm_resource_group.resourcegroup.name}"
  network_interface_ids         = ["${module.proxyserver_nic.id}"]
  vm_size                       = "Basic_A0"
  delete_os_disk_on_termination = "true"

  storage_os_disk {
    name              = "${var.env}ProxyOsDisk"
    caching           = "None"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
  ...
  tags {
    environment = "${var.env}"
    info        = "${var.info_tag}"
    note        = "Proxy server, running NGINX"
  }
}

resource "azurerm_virtual_machine_extension" "proxyvmext_bootstrapnginx" {
  name                 = "${var.env}proxyserver"
  location             = "${azurerm_resource_group.resourcegroup.location}"
  resource_group_name  = "${azurerm_resource_group.resourcegroup.name}"
